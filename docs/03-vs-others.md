...
> **공정한 비교 + 갈아타기 가이드**
>
> T사와 Lonit은 비슷한 일을 하지만 **방식**이 다릅니다. 어느 쪽이 무조건 좋다기보다, 셀러의 상황에 따라 결정이 달라질 수 있습니다.

!!! tip "🎯 이 챕터에서 배우는 것"
    - T사와 Lonit의 핵심 차이점 (인프라·자동화·확장성)
    - Lonit이 잘하는 것 / T사가 잘하는 것 (양쪽 솔직히)
    - 갈아타는 게 효과적인 셀러 특징
    - T사 → Lonit 마이그레이션 단계별 체크리스트
...
| **수집 방식** | 페이지 DOM 파싱 (느림) | API 또는 페이지 파싱 (네트워크·사이트별 다름) |
| **지원 마켓** | 스마트스토어 (주력), 일부 마켓 | 스마트스토어·쿠팡·롯데온·11번가 (4개 동시) |
| **SEO 최적화** | 수동 입력 | 자동 (네이버 쇼핑 상위 상품 기반 추천 태그) |
| **카테고리 매핑** | 단순 문자열 매칭 | 다단계 자동 (DB 학습 + 키워드/상품명 분석 + 네이버 검색) |
| **가격 동기화** | 수동 트리거 | 정기 스케줄러 + 정책 변경 시 즉시 push |
...
상품 1개 → 4마켓 등록:
- **T사**: 마켓별 폼을 따로 채워야 함 (수동 작업 시간 큼)
- **Lonit**: 한 번 등록 → 4마켓 동시 시작 (마켓 응답·검수 시간에 따라 완료 시각 다름)
...
sequenceDiagram
    ...
    Note over Lonit: 정기 스케줄러
    
    MUSINSA->>MUSINSA: 가격 24,000 → 22,000
    Lonit->>MUSINSA: 가격 확인
    MUSINSA-->>Lonit: 22,000원
    Lonit->>Lonit: 정책 적용 → 새 판매가 계산
    Lonit->>MK: 4마켓 가격 일괄 변경
    MK-->>Lonit: ✅ 반영됨
    
    Note over MK: 셀러는 아무것도 안 함
```

T사는 **수동**으로 가격 동기화를 트리거해야 합니다. Lonit은 정기 스케줄러로 자동.

### 2-4. 효율적인 수집

| 작업 | T사 | Lonit |
|------|--------|-------|
| 수집 방식 | 페이지 DOM 파싱 위주 | 사이트별 API 또는 파싱 (12개 사이트 대응) |
| 셀러 작업 | 페이지 일일이 클릭 | 익스텐션 한 번 클릭 또는 일괄 수집 |

수집 속도는 사이트 응답·네트워크 상태에 따라 달라집니다. Lonit은 12개 사이트의 구조를 미리 학습해 두어 추가 작업 없이 자동 추출됩니다.
...
균형 있는 비교를 위해 T사의 강점도 인정해야 합니다.

### 3-1. 오랜 운영 안정성

T사는 **2018년부터** 운영. 수많은 셀러가 사용해 왔고, 엣지 케이스가 다 발견·수정된 상태. 신규 도구는 아무리 잘 만들어도 운영 시간만큼은 따라잡을 수 없습니다.

### 3-2. 익숙한 UI

T사를 쓰던 셀러는 **메뉴 위치·용어·흐름**에 익숙합니다. Lonit으로 갈아타면 처음엔 헷갈릴 수 있습니다 — 이 챕터의 [4. 마이그레이션 가이드](#4)가 도움이 될 것입니다.

### 3-3. 다중 무신사 계정 운영

T사는 한 셀러가 **여러 무신사 계정**을 한 화면에서 운영하는 기능이 매끄럽게 정리되어 있습니다. Lonit도 가능하지만, T사만큼 UI가 다듬어지진 않았습니다 (Phase 2 로드맵).

### 3-4. PHP 기반의 단순함

T사는 PHP 기반이라 **단순한 구조**입니다. 디버깅이나 수정이 직관적인 면이 있습니다. (반대로 Lonit은 더 정교한 기능을 위해 더 복잡한 스택을 씁니다 — TypeScript + 멀티테넌트 PostgreSQL 등)

---

## 4. T사 → Lonit 마이그레이션 가이드 { #4 }

T사를 쓰고 계셨다면, 이 섹션이 갈아타기에 도움이 됩니다.
...
flowchart TB
    Q1{현재 운영 중<br>마켓 수는?}
    Q1 -->|1개 MK (스마트스토어 only)| A1[Lonit 효과 보통]
    Q1 -->|2~4개 MK| A2[⭐ Lonit 효과 큼<br>4마켓 동시 운영 + 송장 자동 분배]
...
!!! warning "직접 import 기능은 현재 없습니다"
    T사에서 등록한 상품 정보를 Lonit으로 일괄 import하는 기능은 아직 제공하지 않습니다. 두 가지 방법:
...
- [ ] Lonit 가입 + 작업 공간 확인
- [ ] 네이버·쿠팡·11번가 셀러센터에 서버 IP `158.247.214.201` 등록 (롯데온은 별도 proxy 환경 사용 가능 — Lonit 운영진 문의)
- [ ] 4마켓 API 키 발급 + Lonit에 등록 + 연결 테스트 ✅
- [ ] 무신사 계정 연결
- [ ] 가격 정책 1개 설정 (마진 30%, 최소 5,000원 권장)
- [ ] 익스텐션 설치 + 동작 확인
- [ ] 테스트 상품 1개 4마켓 업로드 → 결과 확인
- [ ] 자동 동기화 동작 확인 (정책 변경 또는 다음 사이클 대기)
- [ ] 첫 주문 발생 시 송장 자동 분배 확인
- [ ] 1~2주 병행 운영 후 만족 시 T사 단계 축소
...
- ⏸️ 스마트스토어 1개만 운영
- ⏸️ 매일 5건 미만 등록
- ⏸️ T사에 이미 수년간 쌓인 데이터·노하우가 있고 마이그레이션 시간이 부담
- ⏸️ 새 도구 학습 시간이 곧 매출 손실

### 둘 다 쓰는 경우 (병행 운영)

- 🔁 T사: 메인 1개 마켓 + 익숙한 무신사 계정
...
!!! success "📌 한 줄 요약"
    **2~4개 마켓 / 매일 50건 이상 / SEO 자동화 원함** 중 2개 이상 해당하면 Lonit 효과가 명확. 1~2주 병행 운영으로 안전하게 갈아타세요.
