...
!!! tip "🎯 이 챕터에서 배우는 것"
    - 네이버가 노출 순위를 정할 때 보는 5가지 점수
    - 좋은 상품명 vs 나쁜 상품명 (실제 예시)
    - 태그·카테고리·브랜드 자동화 — Lonit이 무엇을 처리하는가
    - 등록 후 첫 7일을 놓치면 안 되는 이유
...
### 3-2. Lonit이 자동 입력하는 태그

```mermaid
flowchart LR
    PRODUCT[등록할 상품<br>'후드티'] --> N[네이버 쇼핑 검색]
    N --> Seed[상위 5개 상품 분석]
    Seed --> Pool{태그 풀}
    Pool --> T1[남자 빅사이즈]
    Pool --> T2[데일리]
...
```mermaid
flowchart TB
    Start[수집된 상품] --> S1{DB 학습 매칭<br>같은 셀러 과거 등록}
    S1 -->|일치| Found[✅ 카테고리 결정]
    S1 -->|없음| S2{하드코딩 룰<br>주요 카테고리 직접 매핑}
    S2 -->|일치| Found
    S2 -->|없음| S3{키워드 매칭<br>상품명 워드 분석}
    S3 -->|일치| Found
    S3 -->|없음| S4{네이버 검색 추천<br>네이버 쇼핑 검색 활용}
    S4 -->|일치| Found
    S4 -->|없음| Manual[⚠️ 수동 검토 보류<br>셀러에게 알림]
...
**여러 단계로 자동 매핑**됩니다. 모든 단계가 실패한 상품은 수동 검토 보류로 남고 셀러가 직접 카테고리를 지정할 수 있습니다.
...
!!! success "📌 한 줄 요약"
    **스마트스토어 = 검색 SEO**. 상품명에 핵심 키워드 + 자연스러운 한국어, 금지어 회피, 카테고리 정확. **Lonit이 7할 자동화** (제목 다듬기·태그 추천·카테고리 매핑·브랜드 매칭). 셀러는 등록 후 첫 7일 클릭률 끌어올리기에 집중.
