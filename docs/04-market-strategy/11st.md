...
KC 인증(국내 안전 인증)은 일부 카테고리에서 의무 사항입니다.
...
!!! warning "현재 Lonit이 지원하는 범위"
    Lonit은 **일반 패션 카테고리(의류·잡화 등) 기준으로 'KC 인증 없음' 정보를 자동 전송**합니다. KC 인증이 필수인 카테고리(영유아·전자제품·일부 화장품 등)는 현재 셀러가 11번가 셀러오피스에서 **상품 등록 후 인증 정보를 직접 입력**해야 합니다.

```mermaid
flowchart LR
    Cat[카테고리 매핑됨] --> Type{KC 필요 카테고리?}
    Type -->|패션/잡화 등| Auto[KC 없음 자동 입력]
    Type -->|영유아·전자 등| Manual[셀러 직접 추가 필요]
    
    Auto --> Upload[11번가 등록 정상]
    Manual --> SellerOffice[셀러오피스에서 KC 정보 입력]
    SellerOffice --> Upload
    
    style Auto fill:#dcfce7,stroke:#22c55e
    style Manual fill:#fef3c7,stroke:#f59e0b
```

### 2-3. 셀러가 해야 할 일

KC 인증이 필수인 카테고리 상품은 11번가 셀러오피스에서 인증번호 + 인증기관을 등록하세요. (이 부분은 추후 Lonit에서 자동화 예정)
...
!!! success "📌 한 줄 요약"
    **11번가 = 형식 완성도**. 카테고리 4단계 + KC 카테고리 직접 입력(필수 카테고리만) + `prdStatCd=01` 자동 + 상품명 100바이트. 패션은 자동 OK, 영유아·전자는 셀러오피스에서 KC 추가 필요.
