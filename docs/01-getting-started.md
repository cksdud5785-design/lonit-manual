...
!!! abstract "📸 스크린샷 위치 — IP 배너 + 4마켓 계정 카드"
    `docs/assets/screenshots/accounts-with-ip-banner.png` 으로 저장 후 이 admonition 을 `![]()` 으로 교체.
...
flowchart TB
    ...
    style 패션 fill:#fef3c7,stroke:#f59e0b
    style 종합 fill:#dbeafe,stroke:#3b82f6
    style 기타 fill:#fce7f3,stroke:#ec4899
```
...
!!! abstract "📸 스크린샷 위치 — 업로드 진행 화면"
    `docs/assets/screenshots/upload-progress.png` 으로 저장 후 교체.
