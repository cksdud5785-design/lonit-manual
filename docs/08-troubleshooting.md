...
!!! tip "🎯 이 챕터의 사용법"
    - 위 검색창 또는 `/` 키로 키워드 빠르게 찾기
    - 카테고리별 진단 흐름은 §1 트리에서
    - 못 찾은 단어는 [용어 사전](09-glossary.md)에서
    - 그래도 막혔다면 `support@lonit.kr` (영업시간 24h 응답)
...
❌ 다른 셀러에게 보이지 않습니다. 상품·주문·마켓 계정·CS 같은 **영업 데이터**는 셀러별 작업 공간(테넌트)으로 완전 분리됩니다. 단, 카테고리 매핑처럼 학습 효과를 위한 **공통 매핑 데이터는 일부 공유**됩니다 (개인 식별 정보 없음).
